...
const DialogInstructions = () => {
    ...
    const {isOpenInstr, setIsOpenInstr, setDefaultTab } = useDialogInstructions();
    ...
    useEffect(() => {
          ...
          if (isOpenInstr) {
            ...
            const elementsToFocus = [
              ...
              'editableblock', 'calcblock', 'foureblock', 'decisionblock', 'exchangeblock',
              'sectorblock', 'submit-button', 'mochila'
            ];
            const elementId = elementsToFocus[step - 1];
            console.log('elementId', elementId);
            const isElementId = (step < 3 || step > 12) && document.getElementById(elementId); 
            const isElementClassName = (step > 2 && step < 13) && document.querySelectorAll(`.${elementId}`); // Usa el ID o clase del elemento
            ...
            return () => {
              const isElementId = (step < 3 || step > 12) && document.getElementById(elementId);
              const isElementClassName = (step > 2 && step < 12 )&& document.querySelectorAll(`.${elementId}`); // Usa el ID o clase del elemento
 ...
 
    {
      isOpenInstr && (
        <div className='fixed inset-0 z-50 bg-black bg-opacity-50 '>
          <div className={`
              ${(step < 3 || step === 13 )? 'top-[50%] md:top-[38%] md:left-[12%]'
                : (step >= 3  && step < 5 )? 'top-[50%] md:top-[19%] md:left-[14%]'
                : step === 5 ? 'top-[65%] md:top-[19%] md:left-[14%]'
                : step === 6 || step === 7  ? 'top-[11%] md:top-[39%] md:left-[14%]' 
                : step === 8 || step === 9  ? 'top-[60%] md:top-[24%] md:left-[-23%]'
                : step === 10 || step === 11   ? 'top-[11%] md:top-[48%] md:left-[-23%]'
                :step === 12 ? 'top-[11%] md:top-[48%] md:left-0'
                : null
