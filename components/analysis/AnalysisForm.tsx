...
import { handleCreateAnalisys, handleUpdateAnalisys } from "@/actions/analisysActions";
...
const AnalysisForm = (
    {type, mode, data, initialValues}: AnalysisFormProps
) => {
    ...
    useEffect(() => {
        const debouncedFunction = debounce(() => {
            if (cualitativeValues.length > 0) {
                ...
                const valoresNumericos = cualitativeValues.map(objeto => objeto.value);
    ...
    useEffect(() => {
        const debouncedFunction = debounce(() => {
            if (cuantitativeValues.length > 0) {
                // Extraer solo los valores numéricos de la propiedad 'value' de cada objeto
                const valoresNumericos = cuantitativeValues.map(objeto => objeto.value);
...
async function onSubmit(values: any) {
    ...
    return (
        ...
        <div className={`z-50 fixed bottom-5 right-5 bg-primary text-white py-2 px-4 rounded-lg shadow-lg
            ...
            Subido exitosamente!
                ...
                {
                    type === "cual" ? (
                        <CualitativeFields mode={mode} data={data} setCualitativeValues={setCualitativeValues}/>
                    ) : (
                        <CuantitativeFields mode={mode} data={data} setCuantitativeValues={setCuantitativeValues}/>
                    )
                } 
