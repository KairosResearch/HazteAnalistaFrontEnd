  ...
  "inline-flex items-center px-2.5 py-0.5 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2",
  {
    variants: {
      variant: {
        ...
        decisionWatchlist: 
          "border-transparent rounded bg-decisionWatchlist/50",
        desicionLeave:
          "border-transparent rounded bg-desicionLeave/50",
        desicionInvest:
          "border-transparent rounded bg-desicionInvest/50",
  ...
  return (
    <div className={cn(badgeVariants({ variant }), className)} {...props}>
      {(variant === 'fourE' || variant === 'range') && (
        <span className={`inline-block  ${variant === 'range' ? 'h-4 w-4 mr-2' : 'h-3 w-3 mr-1'}  rounded-full ${colorClass} align-middle`} />
      )}
      {props.children}
    </div>
  )
