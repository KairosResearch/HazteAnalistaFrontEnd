...
const ChartDefiPositions = () => {
  ...
  const chartConfig = React.useMemo(() => {
    ...
    const balancesConfig = chartData.reduce(
      (acc, curr) => {
    ...
    <div>
      {
        ...
        chartData.length === 0 && (
          <div>
            No logramos acceder a ninguna posición DeFi en tu portafolio. Esto
            puede ser porque:
            <ol>
              <li>- Debes estar logueado en la plataforma con una wallet</li>
              <li>
                - La wallet debe tener posiciones Defi en la red correspondiente
              </li>
            </ol>
          </div>
        )
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              {network === "ethereum" &&
                                defiPositions &&
                                defiPositions.ethereum.totalBalance.toLocaleString()}
                              {network === "arbitrum" &&
                                defiPositions &&
                              ...
                              {network === "optimism" &&
                                defiPositions &&
                                defiPositions.optimism.totalBalance.toLocaleString()}
                              {network === "base" &&
                                defiPositions &&
                                defiPositions.base.totalBalance.toLocaleString()}
                            </tspan>
                            <tspan
                              x={viewBox.cx}
                              y={(viewBox.cy || 0) + 24}
