...
const DefiPosition = (
  ...
  return (
    <>
        ...
        {
          arrayType1 && arrayToShow && arrayToShow.map((position, index) => {
            return(
                <TableRow key={index} className=''>
                  ...
                  <div key={index} className='flex gap-1 items-center '>

                    {
                        position[0].map((pos, index) => {
                            ...
                            <div key={index}>
                                    <Image className="dark:block hidden" src={pos.positions.icon_url ? pos.positions.icon_url.url : '/kairos-main.svg'} alt="Kairos" width={30} height={30} />
                                    <Image className="dark:hidden block" src={pos.positions.icon_url ? pos.positions.icon_url.url : '/kairos-logo-light.svg'} alt="Kairos" width={30} height={30}></Image>
                  ...
                  <TableCell className='w-1/3 flex flex-col ml-24  text center'>
                    {
                        position[0].map((pos, index) => {
                            return(
                                <span key={index}>
                                    {pos.positions.monto_loked.toFixed(3).toLocaleString()} {pos.positions.simbolo}
                                </span>
                            )
                        })
                  ...
                  <TableCell className='w-1/3 text center'>
                    ...
                    (() => {
                        ...
                        return (
                            <>
                            ${totalFiatValue.toFixed(2).toLocaleString()}
                            </>
                        );
                    })()
                    }
        ...
        {
            arrayType2 && arrayToShow && arrayToShow[0].map((position, index) => {
                return(
                    <>
                      ...
                      <TableCell className='w-1/3'>
                      <Badge className=' text-xs absolute top-2 left-0' variant={'Defi'}> 
                            {position[0].positions.position_type === 'deposit' ? 'Deposito' : 
                             position[0].positions.position_type === 'loan' ? 'Prestamo' : 
                             position[0].positions.position_type === 'reward' ? 'Recompensa' : 
                             'Staked'}
                        </Badge>
