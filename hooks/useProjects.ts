...
export const useProjects = (userId: number) => {
  ...
  const { data, error, mutate } = useSWR(["getProjects", userId], () =>
    handleGetProyects(userId),
  );
  ...
  const {
    ...
    needAnalysis && Array.isArray(data) ? ["getAnalysis", data] : null,
    async () => {
      if (!Array.isArray(data)) return [];

      const projectsWithAnalisys = await Promise.all(
        data.map(async (project: any) => {
          const analysis = await handleGetSingleAnalysis(
            project.id_analisis_cualitativo,
            project.id_analisis_cuantitativo,
          );
